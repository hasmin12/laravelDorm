...
class MobileResidentController extends Controller
{
    ...
    public function createMaintenance(Request $request,$id)
    {
        try {
            ...
            $notification = Notification::create([
                ...
                'message' => $user->name." requested a Maintenance"
            ]);
    ...
    public function myPaymentHistory($id)
    {
        $user = User::find($id);
        try {
            if ($user->branch === "Dormitory"){
                $paymentHistory = Dormitorypayment::where('user_id',$id)->get(); 
            }else{
                $paymentHistory = Hostelpayment::where('user_id',$id)->get(); 
            }
    ...
    public function createLaundrySchedule(Request $request)
{
    ...
    public function sendLogs(Request $request, $id)
    {
        $ldate = date('Y-m-d H:i:s');
        $user = User::find($id);
        Log::info($request);
        ...
        try {
            ...
            $img_path = '/storage/' . $path;
                ...
                $residentlog = Residentlog::find($logId);
                $residentlog->update([
                    'return_date' => $ldate,
                ]);
            }
