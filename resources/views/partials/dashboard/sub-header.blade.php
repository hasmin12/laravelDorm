            ...
            <div class="col-md-12">
                <div class="d-flex justify-content-between align-items-center flex-wrap">
                    <div>
                        ...
                        <p>{{ \Illuminate\Support\Str::title(Auth::user()->user_type) }}</p>
                    </div>
                    @if (Auth::user()->user_type == 'admin')
                        <div>
                            <button id="notifyResidents" class="btn btn-link btn-soft-light" style="color: white">
...
<script>
    document.getElementById('notifyResidents').addEventListener('click', function(e) {
        e.preventDefault(); // Prevent default link behavior
        Swal.fire({
            title: 'Are you sure?',
            text: 'Do you want to notify the residents?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                // Logic to notify residents
                // For example, submit a form or make an AJAX call
                window.location.href = '/admin/notify';
                Swal.fire('Notified!', 'The residents have been notified.', 'success');
            }
        });
    });
</script>
