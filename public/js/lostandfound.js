...
function setupCreateLostItemForm() {
    const createLostItemForm = document.getElementById('createLostItemForm');
    createLostItemForm.addEventListener('submit', function (event) {
        event.preventDefault();

        const formData = new FormData(createLostItemForm);
        ...
        fetch('/lostitems', {
            ...
            .then(data => {
                ...
                fetchLostItems(); // Refresh the displayed lost items
            })
            .catch(error => console.error('Error creating lost item:', error));
    });
}
